...
def create_model(fingerprint_input, model_settings, model_architecture,
  ...
  elif model_architecture == 'tiny_conv': # Done (good score)
    return create_tiny_conv_model(fingerprint_input, model_settings,
                                  is_training)
  elif model_architecture == 'tiny_deep_conv': # Done (ok score)
    return create_tiny_deep_conv_model(fingerprint_input, model_settings,
                                            is_training)
  elif model_architecture == 'tiny_deep_high_stride_conv': # Done (bad score)
    return create_tiny_deep_high_stride_conv_model(fingerprint_input, model_settings,
                                            is_training)                                          
...
def create_tiny_deep_conv_model(fingerprint_input, model_settings,
                                     is_training):
  """Builds a convolutional model aimed at microcontrollers.
  ...
  second_bias = tf.compat.v1.get_variable(
      ...
      initializer=tf.compat.v1.zeros_initializer,
      shape=[second_filter_count])
  second_conv_stride_x = 2
  second_conv_stride_y = 2
...
def create_tiny_deep_high_stride_conv_model(fingerprint_input, model_settings,
                                     is_training):
  """Builds a convolutional model aimed at microcontrollers.
...
def create_tiny_conv_1stride_model(fingerprint_input, model_settings, is_training):
  """Builds a convolutional model aimed at microcontrollers.
...
def create_tiny_conv_16filters_model(fingerprint_input, model_settings, is_training):
  """Builds a convolutional model aimed at microcontrollers.
...
def create_tiny_conv_32filters_model(fingerprint_input, model_settings, is_training):
...
def create_tiny_conv_3layer_model(fingerprint_input, model_settings,
                                     is_training):
  """Builds a convolutional model aimed at microcontrollers.
  ...
  third_bias = tf.compat.v1.get_variable(
      name='third_bias',
      initializer=tf.compat.v1.zeros_initializer,
      shape=[third_filter_count])
  third_conv_stride_x = 2
  third_conv_stride_y = 2
  third_conv = tf.nn.conv2d(
      input=second_dropout, filters=third_weights,
      strides=[1, third_conv_stride_y, third_conv_stride_x, 1],
      padding='SAME') + third_bias
  third_relu = tf.nn.relu(third_conv)
  if is_training:
    third_dropout = tf.nn.dropout(third_relu, rate=dropout_rate)
  else:
    third_dropout = third_relu

  third_dropout_shape = third_dropout.get_shape()
  third_dropout_output_width = third_dropout_shape[2]
  third_dropout_output_height = third_dropout_shape[1]
  third_dropout_element_count = int(third_dropout_output_width *
                                     third_dropout_output_height *
                                     third_filter_count)
  flattened_third_dropout = tf.reshape(third_dropout,
                                        [-1, third_dropout_element_count])

  label_count = model_settings['label_count']
  final_fc_weights = tf.compat.v1.get_variable(
      name='final_fc_weights',
      initializer=tf.compat.v1.truncated_normal_initializer(stddev=0.01),
      shape=[third_dropout_element_count, label_count])
  final_fc_bias = tf.compat.v1.get_variable(
      name='final_fc_bias',
      initializer=tf.compat.v1.zeros_initializer,
      shape=[label_count])
  final_fc = (
      tf.matmul(flattened_third_dropout, final_fc_weights) + final_fc_bias)
  if is_training:
    return final_fc, dropout_rate
  else:
    return final_fc

def create_tiny_deep_pooling_model(fingerprint_input, model_settings, is_training):
  """Builds a tiny model with pooling
